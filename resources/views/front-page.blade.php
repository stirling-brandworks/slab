    ...
    @while (have_posts())
        ...
        <section class="pt-2 pb-4 bg-white">
            <div class="container">
                ...
                @if ($quicklinksSec['title'])
                    <div
                        class="title-wrapper text-center @if ($quicklinksSec['link']) w-100 d-md-flex justify-content-md-between align-items-md-center text-md-start @endif">
                        <h3>{!! $quicklinksSec['title'] !!}</h3>
                        @if ($quicklinksSec['link'])
                            <a href="{!! $quicklinksSec['link']['url'] !!}"
                                class="slab-link slab-link--arrow">{!! $quicklinksSec['link']['title'] !!}</a>
                        @endif
                    </div>
                @endif
        ...
        <section class="pt-3 bg-white bg-w-bottom bg-w-bottom--light">
            <div class="container position-relative z-1">
                @if ($books['title'])
                    <div
                        class="title-wrapper text-center @if ($books['link']) w-100 d-md-flex justify-content-md-between align-items-md-center text-md-start @endif">
                        <h3 class="text-center">{!! $books['title'] !!}</h3>
                        @if ($books['link'])
                            <a href="{!! $books['link']['url'] !!}"
                                class="slab-link slab-link--arrow">{!! $books['link']['title'] !!}</a>
                        @endif
                    </div>
                @endif
        ...
        <section class="pt-3 bg-light">
            <div class="container">
                @if ($resources['title'])
                    <div
                        class="title-wrapper text-center @if ($resources['link']) w-100 d-md-flex justify-content-md-between align-items-md-center text-md-start @endif">
                        <h3 class="text-center">{!! $resources['title'] !!}</h3>
                        @if ($resources['link'])
                            <a href="{!! $resources['link']['url'] !!}"
                                class="slab-link slab-link--arrow">{!! $resources['link']['title'] !!}</a>
                        @endif
                    </div>
                @endif
                ...
                @if ($featuredResources)
                    <div class="mt-4 px-lg-5">
                        <div class="swiper-container slab-multi-swiper mx-5 mx-md-0">
                            ...
                            <div
                                class="swiper-button-prev slab-swiper__button-prev slab-swiper__button-prev--dark transition-all ease-ease duration-1">
                            </div>
                            <div
                                class="swiper-button-next slab-swiper__button-next slab-swiper__button-next--dark transition-all ease-ease duration-1">
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="container">
                ...
                <div class="col-lg-5">
                    @php
                        ...
                        $args = ['numberposts' => 3, 'exclude' => $excludeFeaturedPost, 'category' => $categoryIds];
                    ...
                    @if ($news['link'])
                        <div class="mt-3">
                            <a href="{!! $news['link']['url'] !!}"
                                class="btn btn-primary d-block">{!! $news['link']['title'] !!}</a>
                        </div>
                    @endif
                </div>
            </div>
